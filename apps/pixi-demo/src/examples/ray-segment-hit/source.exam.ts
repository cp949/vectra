 ...
 * Ray Segment Hit
 *
 * 화면 고정 emitter에서 쏜 빔(forward ray, t ≥ 0)의 방향을 aim handle로 돌리면, 그 빔이 고정 벽
 * segment를 가로지르는지 매 프레임 boolean으로 판정한다. 빔이 벽에 닿으면 hit 색, 비껴가면 clear
 * 색으로 바뀌어 "이 시선/레이저 빔이 벽에 막히는가?"라는 line-of-sight 작업 흐름을 보인다. ray는
 * forward(t ≥ 0)만 뻗으므로 빔을 벽 반대로 돌리면 backward 연장선이 벽을 지나도 hit=no다.
 *
 * - Intersects.intersectsRaySegment: ray와 segment가 만나면 true를 반환한다. 평행(서로 다른 직선)이면
 *   false, collinear이고 구간이 겹치면 true, 끝점만 닿아도 true(closed)이며, direction이 zero-vector인
 *   degenerate ray는 점으로 환원해 segment의 containment로 판정한다. boolean을 직접 반환해 *Into
 *   companion이 없으므로 그대로 호출한다.
 */
...
export function setup(runtime: PixiRuntime): () => void {
  ...
  const origin: XY = { x: 140, y: 230 };
  // 빔 방향을 정하는 주 drag 대상. direction = aim − origin.
  const aim: XY = { x: 470, y: 150 };
  // 고정 벽 segment (조작 대상 아님). 빔 경로를 가로지르도록 비스듬히 배치.
  const wall: Segment = { a: { x: 430, y: 90 }, b: { x: 560, y: 370 } };
  ...
  const render = (): void => {
    ...
    // 핵심 호출: forward ray가 벽 segment를 가로지르면 true (aim을 emitter에 겹치면
    // direction zero → 점으로 환원돼 containment 판정되는 degenerate)
    const hit = Intersects.intersectsRaySegment(ray, wall);
    ...
    // 벽 segment: 굵은 stroke + 양 끝점 dot
    g.moveTo(wall.a.x, wall.a.y).lineTo(wall.b.x, wall.b.y).stroke({ color: stateColor, width: 5, alpha: 0.95 });
    g.circle(wall.a.x, wall.a.y, 4).fill({ color: stateColor });
    g.circle(wall.b.x, wall.b.y, 4).fill({ color: stateColor });

    // forward 빔: origin → beamEnd (벽 반대로 돌리면 이 선이 벽을 지나지 않아 hit=no)
