...
import { PIXI_RUNTIME_MODULE_SPECIFIERS } from './src/sandbox/pixi-module-specifiers';
...
function vectraSandboxPlugin(): Plugin {
  ...
  return {
    ...
    async load(id) {
      ...
      const imports = PIXI_RUNTIME_MODULE_SPECIFIERS.map((specifier, index) => {
        return `import * as m${index} from ${JSON.stringify(specifier)};`;
      });
      const moduleEntries = PIXI_RUNTIME_MODULE_SPECIFIERS.map((specifier, index) => {
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      {
        find: /^@cp949\/vectra\/([^/]+)$/,
        replacement: `${vectraSourceDir}$1/index.ts`,
      },
      {
        find: /^@cp949\/vectra\/([^/]+)\/(.+)$/,
        replacement: `${vectraSourceDir}$1/$2.ts`,
      },
    ],
  },
});
