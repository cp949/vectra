...
/**
 * target의 aspect ratio를 유지하면서 container 내부에 fit한 rect를 out에 기록한다.
 *
 * contain 모드 결과로, 한 축은 container와 같고 다른 축은 더 짧다. 결과는 container
 * 중심에 정렬된다. target 또는 container가 empty(`width <= 0` 또는 `height <= 0`)이면
 * out에 container를 그대로 복사한다.
 ...
 */
export function fitInsideInto<Out extends RectWritable>(out: Out, target: RectLike, container: RectLike): Out {
  ...
  // contain: 더 작은 scale로 target 전체가 container에 들어오도록 한다
  const scale = targetRatio <= containerRatio ? ch / th : cw / tw;
