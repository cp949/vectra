...
/**
 * cubic Bezier curve를 uniform t로 sampling한 cumulative chord-length lookup table을 out에 push하고 out을 반환한다.
 *
 ...
 * @returns out
 */
export function cubicLookupTableInto(
  ...
  for (let i = 0; i < steps; i++) {
    const t = i / last;
    cubicPointAtTInto(scratch, p0, p1, p2, p3, t);
    const x = scratch.x;
    const y = scratch.y;
    if (i > 0) {
