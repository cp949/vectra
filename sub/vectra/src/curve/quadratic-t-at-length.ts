...
function lengthAtT(
  ...
  for (let i = 0; i < segments; i++) {
    total += integrateSegment(p0x, p0y, p1x, p1y, p2x, p2y, i * step, (i + 1) * step);
  }
  return total;
}

/**
 * quadratic Bezier curve에서 arc length distance에 해당하는 파라미터 t를 반환한다.
 *
 ...
 */
export function quadraticTAtLength(
  p0: XYInput,
  p1: XYInput,
  p2: XYInput,
  distance: number,
  ...
  const totalLength = lengthAtT(p0x, p0y, p1x, p1y, p2x, p2y, 1, segments);
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    const mid = (lo + hi) * 0.5;
    if (lengthAtT(p0x, p0y, p1x, p1y, p2x, p2y, mid, segments) < distance) {
      lo = mid;
    } else {
      hi = mid;
    }
