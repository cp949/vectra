...
/**
 * point list를 'before' mode orthogonal step path로 변환해 out에 기록하고 out을 반환한다.
 *
 * segment p0→p1에서 (x1,y0) elbow를 거쳐 끝점에 도달한다. `stepPathInto`의 mode:'before'와 같다.
 ...
 * @returns out
 */
export function stepBeforePathInto<Out extends PathCommand[]>(out: Out, points: readonly XYInput[]): Out {
  return writeStepPath(out, points, 'before');
}
