...
import { monotoneXPathInto } from './monotone-x-path-into';

/**
 * x축이 strict monotonic인 point list를 monotone cubic Bezier PathCommand[]로 변환한 새 배열을 반환한다.
 *
 ...
 * 성능 최적화가 필요하면 `monotoneXPathInto`를 사용한다.
 *
 * @param points x축이 strict monotonic인 입력 point 배열
 * @returns 새로 만든 PathCommand 배열
 */
export function monotoneXPath(points: readonly XYInput[]): PathCommand[] {
  return monotoneXPathInto([], points);
}
