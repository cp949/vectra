...
/**
 * number tuple/array a와 b를 원소별로 선형 보간하여 out에 기록한다.
 *
 * mutable tuple output을 넘기면 같은 tuple reference type을 그대로 반환하므로
 * TypeScript tuple 길이 정보가 보존된다.
 *
 * @param out  결과를 기록할 mutable tuple/array. error 시 변경하지 않는다.
 ...
 * @returns    out (항상 같은 참조와 같은 type을 반환한다).
 *
 * degenerate 처리:
 * - 빈 tuple/array(a=[], b=[])은 정상 처리하여 out을 비운다.
 ...
 */
export function lerpTupleInto<Out extends number[]>(
  out: Out,
  a: readonly number[],
  b: readonly number[],
  t: number
