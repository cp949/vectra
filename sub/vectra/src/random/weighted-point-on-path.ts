...
import { weightedPointOnPathInto } from './weighted-point-on-path-into';

/**
 * path 위의 무작위 점을 drawing segment별 가중치 분포로 새 object로 반환한다.
 *
 ...
 * 소비 없이 length-uniform으로 배치된다. `weightedPointOnPathInto`에 위임하며 RNG는 segment 선택
 * threshold 계산에 1회만 소비한다. 분포는 default flatten(`flatness=0.5`, `maxRecursion=32`)의
 ...
 *
 * caller 책임: commands에 NaN/Infinity가 있으면 `undefined`를 반환한다. RNG가 `[0, 1)` 범위를
 ...
 */
export function weightedPointOnPath(
  commands: readonly PathCommand[],
  weights: readonly number[],
  rng?: RandomSource
): { x: number; y: number } | undefined {
  const out = { x: 0, y: 0 };
  return weightedPointOnPathInto(out, commands, weights, rng) ? out : undefined;
}
