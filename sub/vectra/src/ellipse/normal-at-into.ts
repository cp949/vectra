import { readEllipseRadiusX, readEllipseRadiusY } from '../internal/ellipse';
import { writeXY } from '../internal/xy';
import type { EllipseLike, XYWritable } from '../types';

/**
 * angle 위치에서 ellipse의 법선 방향 벡터를 out에 기록하고 out을 반환한다.
 *
 * 공식: (cos(angle) / radiusX, sin(angle) / radiusY). 정규화하지 않는다.
 * radiusX <= 0 또는 radiusY <= 0인 empty ellipse는 zero vector (0, 0)을 기록한다.
 *
 * @param out 법선 방향 벡터를 기록할 writable output
 * @param ellipse 법선을 계산할 ellipse
 * @param angle 법선 위치를 나타내는 radian angle
 */
export function normalAtInto<Out extends XYWritable>(out: Out, ellipse: EllipseLike, angle: number): Out {
  const rx = readEllipseRadiusX(ellipse);
  const ry = readEllipseRadiusY(ellipse);

  // empty ellipse이면 zero vector를 기록한다 (공식 적용 시 division-by-zero 발생)
  if (rx <= 0 || ry <= 0) return writeXY(out, 0, 0);

  return writeXY(out, Math.cos(angle) / rx, Math.sin(angle) / ry);
}
