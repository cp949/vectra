 ...
 *
 * empty bounds (`max.x < min.x || max.y < min.y`, inverted/sentinel 모두 포함)는 sentinel empty
 * bounds를 기록한다. 결과는 oriented bounds가 아니라 회전된 corner를 감싸는 axis-aligned bounds다.
 ...
 */
export function transformInto<Out extends BoundsWritable<XYWritable, XYWritable>>(
  out: Out,
  bounds: BoundsLike,
  matrix: MatrixLike
): Out {
  // aliasing 안전: bounds 좌표와 matrix component를 모두 local 변수로 먼저 읽는다
