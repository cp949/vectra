/**
 * SVG `<polyline>` points attribute 문자열을 파싱하여 XYObjectWritable 배열에 기록한다.
 *
 * polygon과 polyline의 SVG points grammar은 동일하다.
...
/**
 * SVG `<polyline>` points attribute 문자열을 파싱하여 out 배열에 기록한다.
 *
 * - out은 호출 전 내용을 보존하지 않는다 (out.length = 0 후 push).
 * - 파싱 실패 시 Error를 throw한다.
 * - 빈 문자열은 정상 처리(out.length = 0).
 * - closed/open 의미는 caller 책임이다 (polygon/polyline 모두 같은 grammar로 파싱).
 *
 ...
 * @returns out (편의 반환)
 */
export function parseSvgPolylineInto(out: XYObjectWritable[], input: string): XYObjectWritable[] {
  out.length = 0;
