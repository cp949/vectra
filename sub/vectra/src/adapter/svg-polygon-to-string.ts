/**
 * XYInput 배열을 SVG `<polygon>` points attribute 문자열로 직렬화한다.
 */

import type { XYInput } from '../types/index';
import { type AdapterStringifyOptions, formatPointList } from './adapter-format.internal';

/** SVG polygon 직렬화 옵션. AdapterStringifyOptions와 동일하다. */
export type SvgPolygonStringifyOptions = AdapterStringifyOptions;

/**
 * XYInput 배열을 SVG `<polygon>` points attribute 형식의 문자열로 직렬화한다.
 *
 * - 빈 배열은 빈 문자열을 반환한다.
 ...
 */
export function svgPolygonToString(points: readonly XYInput[], options?: SvgPolygonStringifyOptions): string {
