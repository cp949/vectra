...
import type { RayWritable, SegmentLike, XYWritable } from '../types';

/**
 * segment `a -> b`를 `origin = a`, `direction = b - a`로 `out`에 기록하고 `out`을 반환한다.
 *
 * zero-length segment는 degenerate ray(direction = 0)가 된다.
 */
export function fromSegmentInto<Out extends RayWritable<XYWritable, XYWritable>>(out: Out, line: SegmentLike): Out {
