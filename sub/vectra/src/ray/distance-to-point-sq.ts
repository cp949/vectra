...
/**
 * ray와 `point` 사이 최단 거리의 제곱을 반환한다.
 *
 * `t`를 `[0, Infinity)`로 clamp해 최근접점을 구한다.
 * degenerate ray(`directionLengthSq === 0`)에서는 origin-point 거리의 제곱을 반환한다.
 */
export function distanceToPointSq(ray: RayLike, point: XYInput): number {
