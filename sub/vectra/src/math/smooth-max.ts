...
/**
 * a와 b의 polynomial smooth maximum을 반환한다.
 *
 * `max(a, b) + h * h * k * 0.25` (`h = max(k - |a - b|, 0) / k`).
 * `|a - b| >= k`이면 정확히 `max(a, b)`, `a === b`이면 `a + k * 0.25`를 반환한다.
 * `smoothMin`을 부호 반전한 것과 같으나 public 함수 import 없이 같은 internal helper를 공유한다.
...
export function smoothMax(a: number, b: number, k: number): number {
  ...
  const result = Math.max(a, b) + smoothBlendOffset(a, b, k);

  if (!Number.isFinite(result)) {
    throw new RangeError('smoothMax result must be a finite number');
  }
