...
/**
 * `xMin..xMax` endpoint 포함 균등 grid에서 sampled function `f`의 composite Simpson 1/3 integral을 반환한다.
 *
 * `f`는 function이어야 한다. function이 아니면 `TypeError`.
 * `xMin`/`xMax`는 finite number, `binCount`는 비음의 safe integer여야 한다. 위반 시 `RangeError`.
 * composite Simpson 1/3 rule은 interval 수(`binCount - 1`)가 짝수이고 `>= 2`여야 한다. 즉 `binCount`는
 * `>= 3`인 홀수여야 한다. 위반 시 `RangeError`. 자동 보정은 하지 않는다.
 * `binCount >= 3`에서는 `xMin < xMax`가 필요하다. 위반 시 `RangeError`. spacing
 ...
 * 가중합(`y[0] + 4*y[1] + 2*y[2] + ... + 4*y[n-1] + y[n]`) 매 누적 단계와 최종 `(sum * dx) / 3`
 * 결과는 모두 finite여야 한다. 위반 시 `RangeError`. 결과의 `-0`은 `0`으로 canonicalize한다.
 *
 ...
 * @param binCount sample 개수(grid point 수). safe integer `>= 3`이고 홀수여야 한다(interval 수가 짝수).
 */
export function simpsonIntegral(f: (x: number) => number, xMin: number, xMax: number, binCount: number): number {
  ...
  if (binCount < 3) {
    throw new RangeError(`binCount must be a safe integer >= 3 for Simpson integration, got ${String(binCount)}`);
  }
  const intervalCount = binCount - 1;
  if (intervalCount % 2 !== 0) {
    throw new RangeError(
      `binCount - 1 must be even for composite Simpson 1/3 rule, got binCount=${String(binCount)} (intervalCount=${String(intervalCount)})`
    );
  }
  if (!(xMin < xMax)) {
  ...
  // composite Simpson 1/3: (dx/3) * (y[0] + 4*y[1] + 2*y[2] + ... + 4*y[n-1] + y[n])
  let sum = evaluateAt(f, xMin, 0) + evaluateAt(f, xMax, binCount - 1);
  if (!Number.isFinite(sum)) {
    throw new RangeError(`Simpson partial sum at endpoint pair must be finite, got ${String(sum)}`);
  }
  for (let i = 1; i < binCount - 1; i++) {
    ...
    // 홀수 index는 4, 짝수 index는 2의 weight를 갖는다.
    const weight = i % 2 === 1 ? 4 : 2;
    sum += weight * value;
    if (!Number.isFinite(sum)) {
      throw new RangeError(`Simpson partial sum at index ${i} must be finite, got ${String(sum)}`);
    }
  }

  const result = (sum * dx) / 3;
  if (!Number.isFinite(result)) {
    throw new RangeError(`Simpson integral result must be finite, got ${String(result)}`);
  }
