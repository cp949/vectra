...
/**
 * unit-grid central difference matrix `D`를 `[binCount, binCount]` 크기로 `out`에 기록한다.
 *
 * `binCount`는 비음의 safe integer여야 한다. 위반 시 `RangeError`.
 * row `i ∈ (0, n - 1)`은 두 이웃 sample 사이 central 차분 `[-0.5, +0.5]`를 columns `[i - 1, i + 1]`에 기록한다.
 ...
 */
export function centralDifferenceMatrixInto<Out extends number[][]>(out: Out, binCount: number): Out {
  ...
  fillFiniteDifferenceMatrixInto(out, binCount, (row, i, n) => {
    ...
    if (i === n - 1) {
      // boundary fallback: 마지막 row는 backward one-sided로 대체한다.
      row[n - 2] = -1;
      row[n - 1] = 1;
      return;
    }
