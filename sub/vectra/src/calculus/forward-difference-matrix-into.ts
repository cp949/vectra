...
/**
 * unit-grid forward difference matrix `D`를 `[binCount, binCount]` 크기로 `out`에 기록한다.
 *
 * `binCount`는 비음의 safe integer여야 한다. 위반 시 `RangeError`.
 * row `i ∈ [0, n - 1)`은 두 sample 사이 forward 차분 `[-1, +1]`을 columns `[i, i + 1]`에 기록한다.
 ...
 */
export function forwardDifferenceMatrixInto<Out extends number[][]>(out: Out, binCount: number): Out {
