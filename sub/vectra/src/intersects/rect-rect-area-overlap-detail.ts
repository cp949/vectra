...
import type { AreaOverlapDetail, RectLike } from '../types';
import { axisAlignedAreaOverlapDetail } from './area-overlap-detail.internal';

/**
 * 두 rect의 면적 중첩 관계 detail을 반환한다.
 *
 * boolean `intersectsRectRect`로 손실되는 touch / overlap / contains 구분을 노출한다.
 * 실제 면적 numeric value나 clipping 결과는 담지 않는다.
 * - `none`: 분리되거나 한쪽이 empty rect(width ≤ 0 또는 height ≤ 0)다.
 * - `touch`: 경계에서만 닿는다. corner touch는 점 1개, edge touch는 닿는 선분 양 끝점 2개다.
 * - `overlap`: 양의 면적으로 겹치고 어느 쪽도 다른 쪽을 완전히 포함하지 않는다.
 * - `contains`: 한쪽이 다른 쪽을 완전히 포함한다. 완전히 같은 rect도 `contains`다.
 *
 ...
 * `epsilon`은 경계 분류 임계값이며 finite validation에는 쓰지 않는다. non-finite 좌표/크기는 `none`이다.
 *
 * @param a 첫 번째 rect
 * @param b 두 번째 rect
 * @param epsilon touch/overlap/contains 경계 판정 임계값
 */
export function rectRectAreaOverlapDetail(a: RectLike, b: RectLike, epsilon = DEFAULT_EPSILON): AreaOverlapDetail {
