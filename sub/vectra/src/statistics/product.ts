 ...
 * 누적 product의 finite 여부를 매 step 함께 검증한다. 위반 시 `RangeError`. entry가 finite이므로
 * `0 * Infinity`는 발생하지 않는다. 결과의 `-0`은 `0`으로 canonicalize한다. input 배열은 mutate하지 않는다.
 *
 * @param values 곱을 계산할 number 배열. finite entry로만 구성된다.
 */
export function product(values: readonly number[]): number {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!Number.isFinite(result)) {
      throw new RangeError(`product must be finite, got ${String(result)} at index ${i}`);
    }
  }
