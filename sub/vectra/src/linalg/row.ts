import { rowInto } from './row-into';
import type { MatLike } from './types';

/**
 * matrix의 `rowIndex`번째 row를 복사해 새 `number[]`로 반환한다.
 *
 * matrix는 rectangular nested array여야 한다. ragged matrix는 `RangeError`.
 * 모든 entry는 finite number여야 한다. 위반 시 `RangeError`.
 * `rowIndex`는 `Number.isInteger`이고 `0 <= rowIndex < rows`여야 한다. 위반 시 `RangeError`.
 *
 * @param matrix row를 읽을 matrix
 * @param rowIndex 읽을 row의 0-based index. 비음의 safe integer.
 */
export function row(matrix: MatLike, rowIndex: number): number[] {
  const firstRow = matrix[0];
  const columns = Array.isArray(firstRow) ? firstRow.length : 0;
  const out: number[] = new Array(columns);
  return rowInto(out, matrix, rowIndex);
}
