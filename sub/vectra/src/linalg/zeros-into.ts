...
/**
 * `shape`에 맞는 zero matrix를 `out`에 기록한다.
 *
 ...
 * 성공 시 각 row를 0으로 채우고 `out.length`는 `rows`로, 각 row length는 `columns`로 truncate된다.
 ...
 */
export function zerosInto<Out extends MatWritable>(out: Out, shape: MatrixShape): Out {
