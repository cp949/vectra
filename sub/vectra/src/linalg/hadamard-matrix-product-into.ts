...
/**
 * 두 matrix의 Hadamard(element-wise) product `out[r][c] = a[r][c] * b[r][c]`를 `out`에 기록한다.
 *
 ...
 *
 * @param out Hadamard product를 기록할 writable matrix. target shape에 맞는 capacity가 준비되어 있어야 한다.
 * @param a Hadamard product의 첫 번째 matrix
 * @param b Hadamard product의 두 번째 matrix. `a`와 같은 shape여야 한다.
 */
export function hadamardMatrixProductInto<Out extends MatWritable>(out: Out, a: MatLike, b: MatLike): Out {
