...
/**
 * `shape`에 맞는 matrix를 `value`로 채워 `out`에 기록한다.
 *
 ...
 * 성공 시 각 row를 `value`로 채우고 `out.length`는 `rows`로, 각 row length는 `columns`로 truncate된다.
 ...
 */
export function fillInto<Out extends MatWritable>(out: Out, shape: MatrixShape, value: number): Out {
