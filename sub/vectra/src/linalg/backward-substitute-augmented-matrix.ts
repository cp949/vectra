...
/**
 * `[U | b]` augmented matrix에 대해 upper triangular backward substitution을 수행해 해 `x`를
 * 새 `number[]`로 반환한다. singular(diagonal 중 하나라도 abs가 `epsilon` 이하)이면 `undefined`를
 * 반환한다.
 *
 * 마지막 column을 RHS `b`로 보고 앞 `rows`개 column을 coefficient `U`로 본다. row `i = n-1..0`
 ...
 * `epsilon`은 diagonal zero 판정과 coefficient 영역의 lower(`column < row`) zero 판정에만 쓰인다.
 * coefficient 영역의 lower에 abs가 `epsilon`보다 큰 entry가 있으면 upper triangular이 아니므로
 * `RangeError`를 던진다.
 * substitution 도중 누적 합 또는 division 결과가 finite number가 아니면 `RangeError`.
 * 결과 entry에는 `-0`이 남지 않는다. 결과는 input 참조를 공유하지 않는 새 `number[]`다.
 *
 * @param augmented `[U | b]` shape의 augmented matrix. 마지막 column이 RHS다.
 * @param options pivot 옵션. `epsilon` 미지정 시 default(`1e-9`).
 */
export function backwardSubstituteAugmentedMatrix(augmented: MatLike, options?: PivotOptions): number[] | undefined {
  const epsilon = resolvePivotEpsilon(options, 'options');
  const shape = extractMatrixShape(augmented, 'augmented');
  const [rows, columns] = shape;
  if (columns !== rows + 1) {
    throw new RangeError(
      `backwardSubstituteAugmentedMatrix requires columns === rows + 1, got shape [${rows}, ${columns}]`
    );
  }
  assertFiniteMatrixEntries(augmented, shape, 'augmented');
  return performTriangularSubstitution(augmented, rows, (r) => augmented[r][rows], epsilon, false, 'augmented');
}
